...
function checkOrientation() {
    ...
    if (!warning) return;
    if (typeof touchInput !== 'undefined' && touchInput.isTouchDevice && window.innerHeight > window.innerWidth) {
        warning.style.display = 'block';
    } else {
        warning.style.display = 'none';
    }
}
// checkOrientation will be called after touchInput is defined below
...
class TouchInputManager {
    constructor() {
        ...
        this.shootDx = 0;
        this.shootDy = 0;
        this.SHOOT_MAX_DIST = 55;
        this.SHOOT_DEADZONE = 6;
    ...
    setupZones() {
        ...
        this.joystickCenterX = screenW * 0.15;
        this.joystickCenterY = screenH * 0.72;
        this.shootCenterX = screenW * 0.78;
        this.shootCenterY = screenH * 0.72;
    }
    ...
    _onTouchStart(e) {
        ...
        for (const touch of e.changedTouches) {
            ...
            if (this._isInJoystickZone(tx) && this.joystickId === null) {
                ...
                this.joystickDx = 0;
                this.joystickDy = 0;
                this._updateJoystickFromTouch(tx, ty);
                ...
                this.shootDx = 0;
                this.shootDy = 0;
                this._updateShootJoystick(tx, ty);
    ...
    _updateJoystickFromTouch(tx, ty) {
        const rawDx = tx - this.joystickCenterX;
        const rawDy = ty - this.joystickCenterY;
    ...
    _updateShootJoystick(tx, ty) {
        const rawDx = tx - this.shootCenterX;
        const rawDy = ty - this.shootCenterY;
        const dist = Math.sqrt(rawDx * rawDx + rawDy * rawDy);
        if (dist < this.SHOOT_DEADZONE) {
            this.shootDx = 0;
            this.shootDy = 0;
        } else {
            const clamped = Math.min(dist, this.SHOOT_MAX_DIST);
            this.shootDx = (rawDx / dist) * (clamped / this.SHOOT_MAX_DIST);
            this.shootDy = (rawDy / dist) * (clamped / this.SHOOT_MAX_DIST);
        }
    }
    ...
    _tryFire() {
        ...
        if (this.shootDx === 0 && this.shootDy === 0) return;
        const room = game.currentRoom;
        const angle = Math.atan2(this.shootDy, this.shootDx);
        if (this.shootDx > 0) p.facing = 1;
        else if (this.shootDx < 0) p.facing = -1;
    ...
    _drawJoystick(ctx) {
        ...
        const stickX = cx + this.joystickDx * this.JOYSTICK_MAX_DIST;
        const stickY = cy + this.joystickDy * this.JOYSTICK_MAX_DIST;
        ...
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.arc(cx, cy, this.JOYSTICK_MAX_DIST, 0, Math.PI * 2);
        ctx.stroke();
        ...
        // Stick
        ctx.save();
        ctx.globalAlpha = this.joystickActive ? 0.55 : 0.25;
        ctx.fillStyle = '#fff';
        ctx.beginPath();
    ...
    _drawShootJoystick(ctx) {
        const cx = this.shootCenterX;
        const cy = this.shootCenterY;
        const stickX = cx + this.shootDx * this.SHOOT_MAX_DIST;
        const stickY = cy + this.shootDy * this.SHOOT_MAX_DIST;
        ...
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.arc(cx, cy, this.SHOOT_MAX_DIST, 0, Math.PI * 2);
        ctx.stroke();
        ...
        // Stick
        ctx.save();
        ctx.globalAlpha = this.shootActive ? 0.6 : 0.25;
...
class Player {
    ...
    update(dt, room) {
        ...
        if (this.rollDuration > 0) {
            ...
            if (touchInput && touchInput.isTouchDevice) {
                const tv = touchInput.getMoveVector();
                dx += tv.dx;
                dy += tv.dy;
            }
            ...
            if (touchInput && touchInput.isTouchDevice && touchInput.consumeRoll() && this.rollCooldown <= 0) {
                this.rollDuration = 0.35;
                let rdx = 0, rdy = 0;
                if (touchInput.joystickActive) {
                    rdx = touchInput.joystickDx;
                    rdy = touchInput.joystickDy;
                }
                if (rdx === 0 && rdy === 0) { rdx = this.facing; }
                const rlen = Math.sqrt(rdx * rdx + rdy * rdy) || 1;
                this.rollDir.x = rdx / rlen;
            ...
            let angle;
            if (touchInput && touchInput.isTouchDevice && touchInput.shootActive && (touchInput.shootDx !== 0 || touchInput.shootDy !== 0)) {
                angle = Math.atan2(touchInput.shootDy, touchInput.shootDx);
                if (touchInput.shootDx > 0) this.facing = 1;
                else if (touchInput.shootDx < 0) this.facing = -1;
            } else {
                const mpos = input.getWorldMouse(game.camera);
                angle = angleBetween(this.x, this.y, mpos.x, mpos.y);
...
class Game {
    ...
    drawHUD() {
        ...
        // Skill cooldown (desktop only, mobile uses touch buttons)
        if (!isMobile) {
