...
class InputManager {
    ...
    getWorldMouse(cam) {
        return {
            x: (this.mouse.x - canvas.width / 2) / game.zoom + cam.x,
            y: (this.mouse.y - canvas.height / 2) / game.zoom + cam.y
        };
    }
}
...
class Game {
    ...
    async checkBackend() {
        try {
            ...
            await fetch('http://localhost:8080/api/auth/login', { method: 'GET', signal: ctrl.signal });
            clearTimeout(timer);
            return true;
        } catch (e) {
            return false;
    ...
    update(dt) {
        ...
        this.zoom = Math.min(canvas.width / 720, canvas.height / 480) * 0.9;
        this.camera.x = lerp(this.camera.x, targetCamX, 8 * dt);
        this.camera.y = lerp(this.camera.y, targetCamY, 8 * dt);
        const halfVW = canvas.width / (2 * this.zoom);
        const halfVH = canvas.height / (2 * this.zoom);
    ...
    drawHUD() {
        ...
        ctx.fillText(`💰 ${this.coins}`, canvas.width - pad, pad + 14);
        ctx.fillStyle = '#e040fb'; ctx.fillText(`💎 ${this.gemsThisRun}`, canvas.width - pad, pad + 34);
        ctx.fillStyle = '#aaa'; ctx.fillText(`层数: ${this.floor}`, canvas.width - pad, pad + 54);
        ctx.fillText(`击杀: ${this.kills}`, canvas.width - pad, pad + 74);
        
        // Weapon
        ctx.fillStyle = '#fff'; ctx.textAlign = 'left'; ctx.font = '13px sans-serif';
        const w = p.currentWeapon;
        ctx.fillText(`[${p.weaponIndex + 1}/${p.weapons.length}] ${w.fullName}`, pad, canvas.height - pad - 20);
        ctx.fillStyle = w.category === 'melee' ? '#aaa' : p.energy >= w.energyCost ? '#8bc34a' : '#f44336';
        ctx.fillText(w.category === 'melee' ? '近战' : `能耗 ${w.energyCost}`, pad, canvas.height - pad - 4);
        
        // Skill cooldown
        const cdRatio = Math.max(0, p.rollCooldown / (2.5 * p.rollCdMult));
        ctx.fillStyle = 'rgba(0,0,0,0.5)';
        ctx.fillRect(canvas.width - 70, canvas.height - 70, 55, 55);
        ctx.fillStyle = cdRatio > 0 ? '#555' : '#e94560';
        ctx.fillRect(canvas.width - 68, canvas.height - 68, 51, 51);
        ctx.fillStyle = '#fff'; ctx.font = 'bold 12px sans-serif'; ctx.textAlign = 'center';
        ctx.fillText('翻滚', canvas.width - 42, canvas.height - 38);
        if (cdRatio > 0) {
            ctx.fillStyle = 'rgba(0,0,0,0.6)';
            ctx.fillRect(canvas.width - 68, canvas.height - 68, 51, 51 * cdRatio);
            ctx.fillStyle = '#fff';
            ctx.fillText(Math.ceil(p.rollCooldown) + 's', canvas.width - 42, canvas.height - 22);
        }
        ...
        if (bossRoom && this.currentRoom && bossRoom !== this.currentRoom) {
            ...
            if (gDist <= 2) {
                ...
                ctx.fillText(warnText, canvas.width / 2, canvas.height - 15);
            }
        }
